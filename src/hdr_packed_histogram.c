...
struct hdr_packed_histogram
{
    ...
    int32_t  last_index;         /* last flat index touched by record (-1 = none) */
    int32_t  last_pos;           /* its position in idx[] (last-hit write cache) */
...
/* Add `delta` into the count already stored at slot `p` (a known hit position),
   widening the uniform width if the sum no longer fits. Shared verbatim by the
   binary-search hit branch of sparse_add and by the record last-hit fast path so
   both have identical width/overflow behavior. Returns false only on a widen
   realloc failure, leaving the slot unchanged (consistent histogram). */
static bool sparse_hit_add(struct hdr_packed_histogram* h, int32_t p, int64_t delta)
{
    /* cur + delta cannot overflow: record_values already refuses a total
       ...
       (monotonic index -> always the miss path), never this hit path. */
...
/* Insert-or-add `delta` at virtual index `vindex`. Returns the slot position in
   idx[]/cnt[] where the count now lives (so the caller can seed its last-hit
   cache), or -1 on allocation failure. */
static int32_t sparse_add(struct hdr_packed_histogram* h, int32_t vindex, int64_t delta)
{
    /* delta is >= 0 (record rejects count<0; decode values are non-negative). */
    int32_t p = lower_bound(h, vindex);
    if (p < h->size && h->idx[p] == vindex)
    {
        return sparse_hit_add(h, p, delta) ? p : -1;
    }
    if (!ensure_cap(h))
    {
        return -1;
                            ...
                            struct hdr_packed_histogram** result)
{
    ...
    h->max_value = 0;
    h->last_index = -1;          /* last-hit write cache: empty */
    h->last_pos = 0;
    *result = h;
    return 0;
}
...
bool hdr_packed_record_values(struct hdr_packed_histogram* h, int64_t value, int64_t count)
{
    ...
    if (count != 0)
    {
        /* Last-hit write cache: bursty/hot streams re-touch the same bucket on
           consecutive records. If this value maps to the exact index we last
           wrote AND that slot still holds it (the idx[] recheck makes the cache
           robust to insert-driven shifts), add in place and skip the binary
           search. Effect is identical to the search path -- sparse_hit_add is the
           same width/overflow-aware add sparse_add's hit branch uses. */
        if (h->last_index == counts_index &&
            h->last_pos < h->size &&
            h->idx[h->last_pos] == counts_index)
        {
            if (!sparse_hit_add(h, h->last_pos, count))
            {
                return false;
            }
        }
        ...
        {
            int32_t p = sparse_add(h, counts_index, count);
            if (p < 0)
            {
                return false;
            }
            h->last_index = counts_index;
            h->last_pos = p;
        }
    }
    h->total_count += count;
...
void hdr_packed_reset(struct hdr_packed_histogram* h)
{
    ...
    h->min_value = INT64_MAX;
    h->max_value = 0;
    h->last_index = -1;          /* invalidate last-hit write cache */
    h->last_pos = 0;
}
    ...
    uint8_t* compressed_histogram, size_t length, struct hdr_packed_histogram** result)
{
    ...
    /* apply zig-zag payload into the sparse structure */
    {
        ...
        while (data_index < counts_limit && counts_index < clen)
        {
            ...
            {
                if (value > 0 && sparse_add(h, counts_index, value) < 0) { ret = ENOMEM; goto done; }
                counts_index++;
            }
        }
    ...
    h->last_index = -1;          /* fresh arrays -> no valid last-hit cache */
    h->last_pos = 0;
    *result = h;
    h = NULL;
